  ...
  <v-footer v-if="signInUser" app class="pa-0">
    <v-bottom-navigation grow dark>
      ...
      <v-menu top open-on-hover offset-y>
        ...
        <v-card>
          <v-list>
            <v-list-item v-for="(item, index) in items" :key="index" link @click="handleMethod(item.methodName)">
              <v-list-item-icon>
...
<script lang="ts">
import { defineComponent, reactive, toRef, toRefs, computed } from '@vue/composition-api';
